...
class MyDatasets(object):
    ...
    def splits(self):
        #train data process
        train_data_image = np.load('/server18/wb/agevoxceleb/multimodal_embedding/best_image_emb/AC/train_final_emb.npy',allow_pickle=True)
        train_data_image_0 = np.zeros((137839,1,256))
        train_data_image_1 = np.concatenate((train_data_image,train_data_image_0),axis=2)# image + zero audio

        train_data_audio = np.load('/server18/wb/agevoxceleb/multimodal_embedding/audio_emb/AC/train_em.npy',allow_pickle=True)
        train_data_audio_0 = np.zeros((137839,1,256))
        ...
        #dev data process
        dev_data_image = np.load('/server18/wb/agevoxceleb/multimodal_embedding/best_image_emb/AC/dev_final_emb.npy',allow_pickle=True)
        dev_data_image_0 = np.zeros((14222,1,256))
        dev_data_image_1 = np.concatenate((dev_data_image,dev_data_image_0),axis=2)

        dev_data_audio = np.load('/server18/wb/agevoxceleb/multimodal_embedding/audio_emb/AC/dev_em.npy',allow_pickle=True)
        dev_data_audio_0 = np.zeros((14222,1,256))
        ...
        #test data process
        test_data_image = np.load('/server18/wb/agevoxceleb/multimodal_embedding/best_image_emb/AC/test_final_emb.npy',allow_pickle=True)
        test_data_image_0 = np.zeros((16027,1,256))
        test_data_image_1 = np.concatenate((test_data_image,test_data_image_0),axis=2)# image + zero audio

        test_data_audio = np.load('/server18/wb/agevoxceleb/multimodal_embedding/audio_emb/AC/test_em.npy',allow_pickle=True)
        test_data_audio_0 = np.zeros((16027,1,256))
