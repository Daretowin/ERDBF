...
from fusion_network import DCCA  as DCCA
...
def parse_args():
    ...
    parser.add_argument('--gpu_id', type=str, default='6', help='which gpu to use')
...
model = DCCA()
model.load_state_dict(torch.load('/server18/wb/Age_Work_Summary/Multimodal_Age_Estimation_2/github/multi_modal/saved_models/DCCA_best.pt'))
...
def validate(model, test_loader):
    ...
    with torch.no_grad():
        ...
        for step,(data0,data1,labels) in enumerate(test_loader):
            ...
            # ---------------------------------------------------------------
            # data0_loss = torch.zeros(1,1,128).cuda()# SC 图像模态丢失，数据置0
            # data1_loss = torch.zeros(1,1,128).cuda()# SC 声音模态丢失，数据置0
